...
class TestApiSec:
    ...
    def test_given_get_company_tickers_exchange_response_when_call_method_then_validate_and_return_json(
        self,
    ):
    ...
    def test_given_company_facts_response_when_call_method_then_validate_and_return_json(
        self,
    ):
        # Prepare api_sec_method to be called
        ...
        # Build mock, call method and assert response
        self._given_json_response_when_call_api_method_then_validate_and_return_json(
            api_method=get_company_facts_method, json_file_path="company_facts_response"
        )

    def test_given_submissions_response_when_call_method_then_validate_and_return_json(
        self,
    ):
        # Prepare api_sec_method to be called
        ...
        # Build mock, call method and assert response
        self._given_json_response_when_call_api_method_then_validate_and_return_json(
            api_method=get_submissions_method, json_file_path="submissions_response"
        )

    @pytest.mark.parametrize("company_concept_enum", list(CompanyConceptEnum))
    def test_given_company_concepts_enum_and_responses_when_call_method_then_validate_and_return_jsons(
        self, company_concept_enum
    ):
        # Prepare api_sec_method to be called
        def get_company_concept_method(
            api_sec: ApiSEC, company_concept: CompanyConceptEnum
        ):
            return api_sec.get_company_concept(
                company_concept=company_concept, cik=1318605
            )
    ...
    def _given_json_response_when_call_api_method_then_validate_and_return_json(
        ...
        mocked_response = mock_response(
            status_code=200, text=get_json_file(json_file_path)
        )
