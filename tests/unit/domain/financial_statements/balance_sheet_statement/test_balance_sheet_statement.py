import pytest
from src.domain.financial_statements.balance_sheet_statement.assets import (
    Assets,
    CurrentAssets,
    NonCurrentAssets,
)
from src.domain.financial_statements.balance_sheet_statement.balance_sheet_statement import (
    BalanceSheetStatement,
)
from src.domain.financial_statements.balance_sheet_statement.liabilities import (
    CurrentLiabilities,
    Liabilities,
    NonCurrentLiabilities,
)
from src.domain.financial_statements.balance_sheet_statement.shareholders_equity import (
    ShareholdersEquity,
)
from src.domain.financial_statements.exceptions.balance_sheet_statement_exception import (
    IsNotBalancedException,
)
...
def build_assets():
    ...
    return Assets(current_assets=current_assets, non_current_assets=non_current_assets)
...
class TestBalanceSheetStatement:
    ...
    def test_given_incorrect_components_when_build_balance_sheet_object_then_it_should_throw_exception(
        self,
    ):
        ...
        # Asserts
        assert (
            exc_info.value.code == "ERROR.FINANCIAL_STATEMENTS.BALANCE_SHEET_STATEMENT"
        )
    ...
    def test_given_variables_when_call_net_working_capital_1_then_it_should_be_calculated(
        self,
    ):
        net_working_capital_1 = BalanceSheetStatement.net_working_capital_1(
            total_current_assets=134836000000, total_current_liabilities=125481000000
        )
        expected_net_working_capital_1 = 9355000000
        # Asserts
        assert net_working_capital_1 == expected_net_working_capital_1

    def test_given_variables_when_call_net_working_capital_2_then_it_should_be_calculated(
        self,
    ):
    ...
    def test_given_variables_when_call_net_working_capital_3_then_it_should_be_calculated(
        self,
    ):
