import pytest
from src.domain.financial_statements.cash_flow_statement.cash_flow_statement import (
    CashFlowStatement,
)
...
class TestCashFlowStatement:
    def test_given_components_when_build_cash_flow_then_it_should_be_built_as_expected(
        self,
    ):
        ...
        # Asserts
        assert (
            cash_flow_statement.operating_activities.net_cash_provided_by_operating_activities
            == expected_net_cash_provided_by_operating_activities
        )
        assert (
            cash_flow_statement.investing_activities.net_cash_used_for_investing_activities
            == expected_net_cash_provided_by_investing_activities
        )
        assert (
            cash_flow_statement.financing_activities.net_cash_used_provided_by_financing_activities
            == expected_net_cash_provided_by_financing_activities
        )
        assert (
            cash_flow_statement.net_change_in_cash_flow
            == expected_net_change_in_cash_flow
        )
        assert (
            cash_flow_statement.cash_at_end_of_period == expected_cash_at_end_of_period
        )

    def test_given_variables_when_calculate_capital_expenditure_then_it_should_be_calculated(
        self,
    ):
        cash_flow_statement = CashFlowStatement(
            operating_activities=operating_activities,
        ...
        cash_flow_statement.calculate_capital_expenditure(
            previous_year_investments_in_property_plant_and_equipment=0
        )
    ...
    def test_given_no_capital_expenditure_when_calculate_free_cash_flow_then_it_should_throw_an_exception(
        self,
    ):
        with pytest.raises(NotFoundCapitalExpenditureException) as exc_info:
            ...
            cash_flow_statement.calculate_free_cash_flow(
                interest_expense=0, tax_shield=0
            )
    ...
    def test_given_variables_when_calculate_free_cash_flow_then_it_should_be_calculated(
        self,
    ):
        cash_flow_statement = CashFlowStatement(
            operating_activities=operating_activities,
