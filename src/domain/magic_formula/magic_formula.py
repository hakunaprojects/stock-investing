...
class MagicFormula(ABC):
    ...
    def _rank_magic_formula_items(self):
        ...
        self.items.sort(key=lambda single_item: MagicFormulaItem.ranking_sum_factors, reverse=True)
        index = 1
    ...
    def _set_ranking_values_to_magic_formula_items(self):
        ...
        self.items.sort(key=lambda single_item: MagicFormulaItem.return_on_capital, reverse=True)
        index = 1
        for item in self.items:
            item.ranking_return_on_capital = index
            index += 1
        # Set ranking_earnings_yield and ranking_sum_factors value to MagicFormulaItems
        self.items.sort(key=lambda single_item: MagicFormulaItem.earnings_yield, reverse=True)
        index = 1
        for item in self.items:
    ...
    def _generate_magic_formula_items(self, tickers: str):
        for ticker in tickers:
            ebit = self.calculate_ebit()
            self.items.append(MagicFormulaItem(
                ticker=ticker,
                return_on_capital=self._return_on_capital(ebit),
                earnings_yield=self._earnings_yield(ebit)
            ))

    def _return_on_capital(self, ebit: float) -> float:
        ...
        tangible_capital_employed = self.calculate_net_working_capital() + self.calculate_fixed_assets()
        return ebit / tangible_capital_employed

    def _earnings_yield(self, ebit: float) -> float:
        ...
        return ebit / self.calculate_enterprise_value()
    ...
    def calculate_ebit(self, **kwargs) -> float:
        ...
        spending requeriments (cash expenses not charged against earnings). """
        ebitda = self.get_ebitda()
        maintenance_capex = self.calculate_maintenance_capex()
        return ebitda - maintenance_capex

    def calculate_maintenance_capex(self, **kwargs) -> float:
        return self.get_ebitda()
    ...
    def calculate_net_working_capital(self, **kwargs) -> float:
        ...
        payables, as these are effectively an interest-free loan (short-term interest-bearing debt was excludad from
        current liabilities). """
        return self.get_total_current_assets() - self.get_excess_cash() - self.get_accounts_payable()
    ...
    def calculate_enterprise_value(self, **kwargs) -> float:
        """Enterprise value takes into account both the price paid for an equity stake in a business as well as the
