...
class BalanceSheetStatement:
    ...
    def total_liabilities_and_total_equity(self) -> int:
        ...
        return (
            ...
            + self.shareholders_equity.total_shareholders_equity
        )
    ...
    def check_is_balanced(self):
        ...
        if self.assets.total_assets != self.total_liabilities_and_total_equity():
            raise IsNotBalancedException(
                "Balance sheet is not balanced. Assets must always equal liabilities plus owners' equity."
            )

    @staticmethod
    def net_working_capital_1(
        total_current_assets: int, total_current_liabilities: int
    ):
    ...
    @staticmethod
    def net_working_capital_2(
        total_current_assets: int, cash: int, total_current_liabilities: int, debt: int
    ):
    ...
    def net_working_capital_3(
        ...
        return (
            accounts_receivables + inventory - accounts_payable + other_working_capital
        )
