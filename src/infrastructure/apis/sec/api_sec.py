...
from src.infrastructure.apis.sec.schemas.sec_schemas import (
...
class ApiSEC(Api):
    ...
    def get_company_tickers_exchange(self):
        ...
        validate(
            instance=company_tickers_exchange_data,
            schema=COMPANY_TICKERS_EXCHANGE_SCHEMA,
        )
        return company_tickers_exchange_data

    def get_company_concept(
        self, company_concept: CompanyConceptEnum, cik: Union[int, str]
    ):
        ...
        url = (
            f"https://data.sec.gov/api/xbrl/companyconcept/CIK{_cik_to_ten_digits_str(cik)}/"
